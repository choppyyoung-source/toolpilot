...
export default function SeoAnalyzerWidget({ labels: l = {} }: { labels?: Partial<Labels> }) {
      ...
      {result && checks.length > 0 && (
        <>
          ...
          <div className="mb-6 rounded-lg bg-white p-6 text-center">
            <div className="mb-2 text-sm text-gray-500">{labels.score}</div>
            ...
            <div className="mt-3 text-xs text-gray-400">Analyzed: {result.url}</div>
          </div>
          ...
          <div className="mb-6 grid grid-cols-3 gap-4">
            <div className="rounded-lg bg-white p-4 text-center">
              ...
              <div className="text-xs text-gray-500">{labels.passed}</div>
            </div>
            <div className="rounded-lg bg-white p-4 text-center">
              <div className="text-2xl font-bold text-yellow-600">
                {checks.filter((c) => c.status === "warn").length}
              </div>
              <div className="text-xs text-gray-500">{labels.warnings}</div>
            </div>
            <div className="rounded-lg bg-white p-4 text-center">
              <div className="text-2xl font-bold text-red-600">
                {checks.filter((c) => c.status === "fail").length}
              </div>
              <div className="text-xs text-gray-500">{labels.failed}</div>
            </div>
          </div>
